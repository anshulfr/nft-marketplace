...
import { Loader, GlobalLoader } from "../../components/index";
import images from "../../assets";
import { shortenAddress } from "../../utils/shortenAddress";

const TransferFunds = ({}) => {
  const { getUserTransferFundHistory, nftCurrency, currentAccount } =
  ...
  // useEffect(() => {
  //   getUserTransferFundHistory().then((item) => {
  //     setTransfer(item);
  //     setIsLoading(false);
  //   });
  // }, []);
  ...
  useEffect(() => {
    const loadNFTs = async () => {
      ...
      try {
        setIsLoading(true);
        const items = await getUserTransferFundHistory();
        setTransfer(items?.reverse() || []);
