...
const community = () => {
  ...
  useEffect(() => {
    Promise.all([communityuserFriendList(), communityAllUser()]).then(
      ([friendList, allUsers]) => {
        //LOG ALL FRINED LIST
        console.log("FRIEND LIST:", friendList);

        //FIND THE USER OWN DATA
        const matchObject = allUsers?.find(
          (obj) =>
            obj.accountAddress.toLowerCase() === currentAccount.toLowerCase()
        );
        console.log("USER:", matchObject);
        setCheckUser(matchObject);

        //FITER OUT THE CURRENT USER FROM THE ALL USERS LIST
        let newList = allUsers?.filter(
          (obj) =>
            obj.accountAddress.toLowerCase() !== currentAccount.toLowerCase()
        );

        // FUTHER FILTER OUT USER WHO ARE ALREADY FRIENDS
        const friendAddress = friendList.map((friend) => friend.pubkey);
        newList = newList.filter(
          (obj) => !friendAddress.includes(obj.accountAddress)
        );
    ...
    <div className="flex justify-center sm:px-4 p-12">
      <div className="w-full minmd:w-4/5">
        <Banner
          name={
            <>
              Welcome to Mintify <br /> Community!
            </>
          }
          ...
          <>
            <div className="mt-10">
              ...
              <div className="mt-3 w-full flex flex-wrap justify-start md:justify-center">
                ...
                {userCopy.length == 0 ||
                  (!currentAccount && (
                    <div className="mt-10 rounded-lg shadow-lg  max-auto block">
                      <img src="/user.jpg" alt="Crypto King" />
                    </div>
                  ))}
              </div>
            </div>
